import {
  a as ToolProviderTools,
  i as ToolProvider,
  s as Executor
} from "./executor-BRm90XJT.js";

//#region src/shared.d.ts
declare const DEFAULT_DESCRIPTION =
  'Execute code to achieve a goal.\n\nAvailable:\n{{types}}\n\nWrite an async arrow function in JavaScript that returns the result.\nDo NOT use TypeScript syntax \u2014 no type annotations, interfaces, or generics.\nDo NOT define named functions then call them \u2014 just write the arrow function body directly.\n\nExample: async () => { const r = await codemode.searchWeb({ query: "test" }); return r; }';
interface CreateCodeToolOptions {
  tools: ToolProviderTools | ToolProvider[];
  executor: Executor;
  /**
   * Custom tool description. Use {{types}} as a placeholder for the generated type definitions.
   */
  description?: string;
}
type CodeInput = {
  code: string;
};
type CodeOutput = {
  result: unknown;
  logs?: string[];
};
/**
 * Normalize the tools option into a list of ToolProviders.
 * Raw ToolSet/ToolDescriptors are wrapped as a single default provider.
 */
declare function normalizeProviders(
  tools: ToolProviderTools | ToolProvider[]
): ToolProvider[];
//#endregion
export {
  normalizeProviders as a,
  DEFAULT_DESCRIPTION as i,
  CodeOutput as n,
  CreateCodeToolOptions as r,
  CodeInput as t
};
//# sourceMappingURL=shared-TyFN6W7_.d.ts.map
