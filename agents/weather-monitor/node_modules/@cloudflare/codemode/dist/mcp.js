...
function sandboxResponseText(content) {
	if (typeof content === "string" && content.length <= MAX_SANDBOX_TRUNCATED_CHARS && content.slice(MAX_CHARS).startsWith(TRUNCATION_FOOTER_PREFIX)) return content;
...
function unwrapMcpResult(result) {
	...
	if (result.isError) {
		const msg = result.content.filter((c) => c.type === "text").map((c) => "text" in c ? c.text : "").join("\n") || "Tool call failed";
		throw new Error(msg);
	}
	if (result.structuredContent != null) return result.structuredContent;
	if (result.content.length > 0 && result.content.every((c) => c.type === "text")) {
...
{{example}}`;
async function codeMcpServer(options) {
	...
	for (const tool of tools) toolDescriptors[tool.name] = {
	...
	for (const tool of tools) {
		...
		fns[toolName] = async (args) => {
			return unwrapMcpResult(await client.callTool({
				name: toolName,
				arguments: args
			}));
		};
	...
	if (firstTool) {
		...
		const parts = [];
		for (const [key, prop] of Object.entries(props)) if (prop.type === "number" || prop.type === "integer") parts.push(`${key}: 0`);
	...
	const codeDescription = (description ?? CODE_DESCRIPTION).replace("{{types}}", types).replace("{{example}}", example);
	const codemodeServer = new McpServer({
		name: "codemode",
		version: "1.0.0"
	});
	codemodeServer.registerTool("code", {
...
`;
function createOpenApiSandboxCode(code, spec, includeRequest) {
	const normalized = normalizeCode(code);
	const specJson = JSON.stringify(spec).replace(/</g, "\\u003c");
	const requestFn = includeRequest ? `request: async (options) => await __openapiHost.request(options)` : "";
	return `async () => {
...
function openApiMcpServer(options) {
	const { executor, request: requestFn, name = "openapi", version = "1.0.0", description } = options;
...
async () => {
  ...
  return results;
		...
		inputSchema: { code: z.string().describe("JavaScript async arrow function to search the spec") }
	}, async ({ code }) => {
		try {
			const result = await executor.execute(createOpenApiSandboxCode(code, spec, false), []);
			if (result.error) return {
				content: [{
					type: "text",
					text: `Error: ${result.error}`
			...
			return { content: [{
				type: "text",
				text: sandboxResponseText(result.result)
			}] };
		} catch (error) {
			return {
				content: [{
...
async () => {
  return await codemode.request({ method: "GET", path: "/your/endpoint" });
	...
	server.registerTool("execute", {
		...
		try {
			const result = await executor.execute(createOpenApiSandboxCode(code, spec, true), [{
				name: "__openapiHost",
				fns: { request: (args) => requestFn(args) }
			}]);
			if (result.error) return {
				content: [{
					type: "text",
...
//#endregion
export { codeMcpServer, openApiMcpServer };
